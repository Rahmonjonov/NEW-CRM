...
function newLeadobject(pk, name, date, summa, company, phone, created_user, validation, validation_date, service_type_name) {
    const LANGUAGE_CODE = document.documentElement.lang || 'uz';
    ...
    let stText       = "Xizmat turi";
    if (LANGUAGE_CODE === "en") { validityText = "Validity period"; phoneText = "Telephone"; stText = "Service type"; }
    else if (LANGUAGE_CODE === "ru") { validityText = "Срок действия"; phoneText = "Телефон"; stText = "Тип услуги"; }
    ...
    div.innerHTML = `
        ...
        <div class="other_lead">
            ${validation_date ? `<div><a class="lead_other_price text-super">${validityText}: ${validation_date}</a></div>` : ''}
            ${phone          ? `<div><a class="lead_other_price text-super">${phoneText}: ${phone}</a></div>`               : ''}
            ${service_type_name ? `<div><a class="lead_other_price text-super">${stText}: <strong>${service_type_name}</strong></a></div>` : ''}
        </div>
...
function updateLead(pk, name, price, company, address, phone, service_type_name) {
    let index = leads_all.findIndex(i => i.id === pk);
    const obj = { id: pk, name, price, company, address, phone, service_type_name: service_type_name || '' };
...
function addLeadToGroupAndUpdateLabels(response) {
    const st_name = response.service_type_name || '';
    for (let i = 0; i < Groups.length; i++) {
        if (Groups[i].dom && Groups[i].dom.id === `group_body_${response.pole}`) {
            const obj = newLeadobject(
                response.id, response.name, response.date,
                response.price, response.company, response.phone,
                response.created_user ? response.created_user.username : '',
                response.get_validity_period, response.validity_period,
                st_name
            );
            updateLead(response.id, response.name, response.price, response.company, response.district, response.phone, st_name);
...
$(document).ready(function () {
    ...
    $('#lead_note_form').submit(function (event) {
        ...
        $.ajax({
            ...
            success: response => { addLeadToGroupAndUpdateLabels(response); $('#lead_note_modal').modal('hide'); }
        });
    });
