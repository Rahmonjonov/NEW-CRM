...
function newLeadobject(pk, name, date, summa, company, phone, created_user, validation, validation_date, service_type_name, district_name, business_type, investment_price, investment_valuta_display, referral_name) {
    ...
    // Call button — telefon bo'lsa chiqadi
    const callBtnHTML = phone
        ? `<a href="tel:${phone}" class="lead_call_btn" onclick="event.stopPropagation();" title="${phone}">
    ...
    let bottomRight = '';
    if (is_B2B()) {
        const refHTML = referral_name
            ? `<div class="lead_note text-super">${referral_name}</div>`
            : '';
        bottomRight = `<div style="display:flex;align-items:center;gap:6px;">${refHTML}${callBtnHTML}</div>`;
    } else {
        bottomRight = `<div style="display:flex;align-items:center;gap:6px;">
                           <a href="/edit/?id=${pk}"><i class="fa fa-info-circle" style="color:#6b778d;"></i></a>
                           ${callBtnHTML}
                       </div>`;
    }
...
$(document).ready(function () {
    ...
    // define_groups_leads() o'chirildi — leadlar AJAX bilan yuklanadi
    ...
    $('#newLeadForm').submit(function (event) {
        ...
        $.ajax({
            type: "POST", url: '/board/create_lead/',
            beforeSend: xhr => xhr.setRequestHeader("X-CSRFToken", csrf_token),
            data: dataBody,
    ...
    $('#lead_note_form').submit(function (event) {
        ...
        removeLead(leadId);
        $.ajax({
            type: "POST", url: '/board/change_lead_status/',
            beforeSend: xhr => xhr.setRequestHeader("X-CSRFToken", csrf_token),
    ...
    $('#lead_finished_form').submit(function (event) {
        ...
        let data = getFormData($(this).serializeArray());
        $.ajax({
            type: "POST", url: '/board/lead_finished/',
            beforeSend: xhr => xhr.setRequestHeader("X-CSRFToken", csrf_token),
    ...
    $('#lead_lose_form').submit(function (event) {
        ...
        let data = getFormData($(this).serializeArray());
        $.ajax({
            type: "POST", url: '/board/lead_losed/',
            beforeSend: xhr => xhr.setRequestHeader("X-CSRFToken", csrf_token),
    ...
    // Sahifa ochilganda applyFilters Board.html da chaqiriladi
});
